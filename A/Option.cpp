...
double CallOption::gamma(double b) const {
    double d1 = compute_d1(data.S, data.K, data.T, b, data.sig);
    double numer = exp((b - data.r) * data.T) * n(d1);
    double denom = data.S * data.sig * sqrt(data.T);
    return numer / denom;
}
...
/**
 * Gamma (same formula as CallOption)
 */
...
/**
 * Put option Delta
 * Always negative (put value decreases as spot increases)
 */
